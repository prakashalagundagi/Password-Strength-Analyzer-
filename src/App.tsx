...
function IconButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white/70 px-3 py-2 text-sm font-medium text-slate-800 shadow-sm backdrop-blur transition",
        "hover:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400/50",
...
export default function App() {
  ...
  useEffect(() => {
    // Auto-suggest a strong password when the current one is weak (debounced)
    const t = window.setTimeout(() => {
      if (!password) return;
      if (analysis.score >= 3) return;
      setSuggested(defaultBestPassword(18));
    }, 650);
  ...
  async function handleCopy(text: string, label: string) {
    try {
      ...
      pushToast("Copy failed (clipboard blocked)");
    }
  }

  const card =
    "rounded-3xl border border-slate-200/70 bg-white/60 p-5 shadow-xl shadow-slate-900/5 backdrop-blur ring-1 ring-white/40";
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Real-time strength check + smart hints — all client-side.</p>
            </div>
          </div>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="hidden sm:block text-right">
                ...
                <div className="text-sm font-semibold text-slate-800">{password ? analysis.crackTime || "—" : "—"}</div>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-3">
              <div className="relative">
                <input
                  ...
                  className={cn(
                    "w-full rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 pr-28 text-base text-slate-900 shadow-sm outline-none backdrop-blur",
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-slate-200 bg-white/60 p-3">
                  ...
                  <div className="mt-0.5 text-sm font-semibold text-slate-900">{password ? scoreLabel(analysis.score) : "—"}</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white/60 p-3">
                  <div className="text-xs text-slate-500">Approx. bits</div>
                  <div className="mt-0.5 text-sm font-semibold text-slate-900">{password ? Math.round(analysis.approxBits) : "—"}</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white/60 p-3">
                  <div className="text-xs text-slate-500">Guesses (log10)</div>
                  <div className="mt-0.5 text-sm font-semibold text-slate-900">{password ? analysis.guessesLog10.toFixed(1) : "—"}</div>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <IconButton
                    ...
                    onClick={() => {
                      ...
                      pushToast("Suggested password applied");
                    }}
                  >
                    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={2}>
                  ...
                  {password ? (
                    improvementHints.length ? (
                      <ul className="list-disc space-y-1 pl-5 text-sm text-slate-700">
                        {improvementHints.slice(0, 6).map((h) => (
                          <li key={h}>{h}</li>
                        ))}
                      </ul>
                    ) : (
                      <div className="text-sm text-slate-700">Looks great — strong and hard to guess.</div>
                    )
                  ) : (
      ...
      <AnimatePresence>
        {toast ? (
          <motion.div
            ...
            className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 rounded-2xl border border-slate-200 bg-white/80 px-4 py-2 text-sm text-slate-800 shadow-xl shadow-slate-900/10 backdrop-blur"
            role="status"
            aria-live="polite"
          >
            {toast.message}
